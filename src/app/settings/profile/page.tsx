...
export default function ProfileSettingsPage() {
  ...
  if (isLoading || isFetching) {
    return (
      <div className="flex min-h-screen w-full items-center justify-center bg-slate-50 dark:bg-zinc-950 text-slate-800 dark:text-zinc-100">
        <div className="flex flex-col items-center gap-3">
          <Loader2 className="h-8 w-8 animate-spin text-cyan-500 dark:text-cyan-400" />
          <p className="text-sm text-slate-500 dark:text-zinc-400">Loading settings...</p>
        </div>
      </div>
    );
  }
  ...
  return (
    <div className="min-h-screen bg-slate-50 dark:bg-[#050508] px-4 py-24 text-slate-800 dark:text-zinc-100 transition-colors duration-300">
      <div className="mx-auto max-w-2xl rounded-2xl border border-slate-200 dark:border-zinc-800/80 bg-white dark:bg-zinc-900/40 p-6 shadow-xl dark:shadow-2xl backdrop-blur-xl md:p-8">
        
        {/* Header */}
        <div className="mb-8 border-b border-slate-200 dark:border-zinc-800/60 pb-6">
          <h1 className="text-3xl font-extrabold tracking-tight text-slate-900 dark:text-zinc-50">
            Speaker Profile Settings
          </h1>
          <p className="mt-2 text-sm text-slate-550 dark:text-zinc-400">
        ...
        {statusMsg && (
          <div className={`mb-6 rounded-lg border p-3.5 text-sm ${
            statusMsg.type === "success" 
              ? "border-emerald-500/20 bg-emerald-500/5 dark:bg-emerald-950/20 text-emerald-600 dark:text-emerald-400" 
              : "border-red-500/20 bg-red-500/5 dark:bg-red-950/20 text-red-600 dark:text-red-400"
          }`}>
            {statusMsg.text}
          </div>
        )}
        ...
        <form onSubmit={handleSave} className="space-y-8">
          ...
          <div>
            ...
            <p className="text-xs text-slate-500 dark:text-zinc-400 mt-0.5">Adjusts prompt complexity and theme parameters.</p>
            <div className="mt-3.5 grid grid-cols-1 gap-3 sm:grid-cols-2">
              {[
                ...
                return (
                  ...
                  >
                    <Icon className={`h-5 w-5 ${isSelected ? "text-cyan-600 dark:text-cyan-400" : "text-slate-400 dark:text-zinc-500"}`} />
                    <span className="mt-3 font-semibold text-slate-900 dark:text-zinc-100 text-sm">{item.label}</span>
                    <span className="mt-1 text-xs text-slate-550 dark:text-zinc-400">{item.desc}</span>
                  </button>
                );
              })}
            </div>
          ...
          <div>
            ...
            <p className="text-xs text-slate-500 dark:text-zinc-400 mt-0.5">Determines dynamic topic prompts difficulty.</p>
              ...
              {[
                ...
                return (
                  ...
                  >
                    ...
                    <div>
                      <span className="font-semibold text-slate-900 dark:text-zinc-100 text-sm">{item.label}</span>
                      <span className="block mt-0.5 text-xs text-slate-550 dark:text-zinc-400">{item.desc}</span>
                    </div>
                  </button>
                );
              })}
          ...
          <div>
            ...
            <p className="text-xs text-slate-500 dark:text-zinc-400 mt-0.5">Highlights stats related to this speaking goal on analysis cards.</p>
            <div className="mt-3.5 flex flex-col gap-2.5">
              {[
                ...
                return (
                  ...
                  >
                    <Icon className={`h-4 w-4 ${isSelected ? "text-cyan-600 dark:text-cyan-400" : "text-slate-400 dark:text-zinc-500"}`} />
                    <div>
                      <span className="font-semibold text-slate-900 dark:text-zinc-100 text-sm">{item.label}</span>
                      <span className="block mt-0.5 text-xs text-slate-550 dark:text-zinc-400">{item.desc}</span>
                    </div>
          ...
          <div>
            ...
            <p className="text-xs text-slate-500 dark:text-zinc-400 mt-0.5">Increments streak tracking parameters on completion.</p>
                ...
                return (
                  ...
                  >
                    <div className="flex items-center gap-3">
                      ...
                      <div>
                        <span className="font-semibold text-slate-900 dark:text-zinc-100 text-sm">{item.label}</span>
                        <span className="ml-2 rounded bg-slate-100 dark:bg-zinc-800 px-1.5 py-0.5 text-[10px] font-medium tracking-wide uppercase text-slate-650 dark:text-zinc-400 border border-slate-200 dark:border-zinc-750">
                          {item.sub}
                        </span>
                        <span className="block mt-1 text-xs text-slate-550 dark:text-zinc-400">{item.desc}</span>
                      </div>
                    </div>
                  </button>
                );
